...
bool CAudioDecoder::Init(const CFileItem& file, unsigned int filecache)
{
  ...
  bool ret = m_struct.toAddon.Init(&m_struct,
...
int CAudioDecoder::ReadPCM(uint8_t* buffer, int size, int* actualsize)
{
  ...
  return m_struct.toAddon.ReadPCM(&m_struct, buffer, size, actualsize);
}
                         ...
                         MUSIC_INFO::EmbeddedArt* art)
{
  ...
  if (!m_struct.toAddon.ReadTag ||
      !m_struct.toAddon.ReadTag(&m_struct, fileName.c_str(), title, artist, &length))
    return false;
...
int CAudioDecoder::GetTrackCount(const std::string& strPath)
{
  ...
  int result = m_struct.toAddon.TrackCount(&m_struct, strPath.c_str());
