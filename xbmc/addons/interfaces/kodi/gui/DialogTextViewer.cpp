...
void Interface_GUIDialogTextViewer::Open(void* kodiBase, const char *heading, const char *text)
{
  ...
  {
    CLog::Log(LOGERROR, "ADDON::Interface_GUIDialogTextViewer::%s - invalid handler data (heading='%p', text='%p', dialog='%p') on addon '%s'", __FUNCTION__, heading, text, dialog, addon->ID().c_str());
    return;
  }
